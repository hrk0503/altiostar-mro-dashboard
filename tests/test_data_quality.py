"""Data quality tests -- validate the official CSVs pass all quality checks."""
import pandas as pd
import pytest
...
class TestNeighborRelationsQuality:
    ...
    def test_no_self_neighbors(self, neighbors_df):
        self_refs = neighbors_df[neighbors_df["serving_cell"] == neighbors_df["neighbor_cell"]]
    ...
    def test_cio_in_range(self, neighbors_df):
        assert (neighbors_df["cell_individual_offset_dB"].between(-15, 15)).all()

    def test_each_cell_has_neighbors(self, neighbors_df, sites_df):
        cells_with_neighbors = set(neighbors_df["serving_cell"].unique())
...
class TestPMDataQuality:
    ...
    def test_ho_success_le_attempts(self, pm_data_df):
        violations = pm_data_df[pm_data_df["ho_success_intra"] > pm_data_df["ho_attempts_intra"]]
        assert len(violations) == 0, f"{len(violations)} rows where ho_success > ho_attempts"

    def test_kpi_ranges(self, pm_data_df):
        assert (pm_data_df["prb_utilization_dl_pct"].between(0, 100)).all()
        assert (pm_data_df["avg_rsrp_dBm"].between(-140, -40)).all()
    ...
    def test_rates_sum_to_100(self, pm_data_df):
        total = pm_data_df["ho_success_rate_pct"] + pm_data_df["ho_failure_rate_pct"]
        assert ((total - 100.0).abs() < 1.0).all(), "success_rate + failure_rate should ~ 100%"
...
class TestClusterKPIQuality:
    ...
    def test_has_problem_cells(self, cluster_kpi_df):
        problem_count = (cluster_kpi_df["problem_cell"] == "Yes").sum()
        assert problem_count > 0, "Expected at least one problem cell"

    def test_failure_rate_matches_counts(self, cluster_kpi_df):
        computed = cluster_kpi_df["total_ho_failures"] / cluster_kpi_df["total_ho_attempts"] * 100
        diff = (cluster_kpi_df["ho_failure_rate_pct"] - computed).abs()
        assert (diff < 0.1).all(), "ho_failure_rate_pct should match failures/attempts"

    def test_problem_cells_have_high_failure(self, cluster_kpi_df):
        problems = cluster_kpi_df[cluster_kpi_df["problem_cell"] == "Yes"]
