#!/usr/bin/env python3
"""Train and evaluate PPO on the Kyiv datasets with an 80/20 temporal train/test split.

This script implements:
1. Chronological splitting of telemetry data (80% train, 20% test).
2. Greedy CIO search on the training split to establish optimal weights.
3. SB3 PPO policy injection and training on the training split.
...
def main():
    ...
    kyiv_dirs = sorted([
        ...
        if d.is_dir() and "kyiv" in d.name and (d / "pm_data_relation_level.csv").exists()
    ])

    print(f"Found {len(kyiv_dirs)} Kyiv datasets")
    all_results = []
    
    for geo_dir in kyiv_dirs:
        try:
    ...
    summary = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "purpose": "Kyiv 80/20 temporal train/test split PPO validation",
        "results": all_results
    }

    results_dir = ROOT / "results"
    results_dir.mkdir(exist_ok=True)
    out_path = results_dir / "kyiv_temporal_evaluation.json"
